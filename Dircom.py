...
def app():
    ...
    Vervolgens worden de **totalen van zendingen** en **totale Laadmeters (LM)** getoond voor alle zendingen en per 'Dienst_Categorie'.
    ...
    if uploaded_file is not None:
        try:
            ...
            with col1:
                st.metric(label="Totaal **Zendingen**", value=f"{total_zendingen:,.0f}") # Aangepast label
            with col2:
                st.metric(label="Totale Laadmeter (LM)", value=f"{total_lm:,.2f} LM")

            st.write("---")

            # --- Totaaloverzicht per Dienst_Categorie (Zendingen en LM) ---
            st.subheader("2. Detail")

            df_summary_by_category = df_grouped.groupby('Dienst_Categorie').agg(
                Zendingen=('Verzending-ID', 'size'), # Aangepast agg key
                Totaal_LM=('LM', 'sum')
            ).reset_index()

            # Hernoem kolommen voor duidelijkheid
            df_summary_by_category.columns = ['Dienst Categorie', '**Zendingen**', 'Totaal LM'] # Aangepaste kolomnaam
            
            # Formatteer de numerieke kolommen voor weergave
            df_summary_by_category['**Zendingen**'] = df_summary_by_category['**Zendingen**'].map(lambda x: f"{x:,.0f}") # Aangepaste kolomnaam
